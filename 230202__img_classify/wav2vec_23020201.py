...
Original file is located at
    https://colab.research.google.com/drive/1FGZ_-nNdoaUDzT2iJ5-uoiK9p4zt7n6c
"""
...
bundle = torchaudio.pipelines.WAV2VEC2_ASR_BASE_960H  # 加载wav2vec模型
model = bundle.get_model().to(device)


# 读取视频，返回音频Mel-Spec
def get_wav2vec_features(path):
    y, sr = librosa.load(path)
    waveform = torch.from_numpy(y)
    waveform = torch.unsqueeze(waveform, dim=0)
    waveform = torchaudio.functional.resample(waveform, sr, bundle.sample_rate)  # resample到模型sample rate
    waveform = waveform.to(device)
    with torch.inference_mode():
        features, _ = model.extract_features(waveform)
    return features[11]  # 返回最后一层features
...
save_model_path = "./wav2model"  # 设置保存模型路径

# RNN模型参数
RNN_hidden_layers = 3
RNN_hidden_nodes = 256  #原来是512
RNN_FC_dim = 128        #原来是256
...
input_size = 768  # wav2vec输出feauture维度为768
...
def validation(model, device, optimizer, test_loader):
    ...
    with open('wav2datas.txt', 'a', encoding='utf-8') as f:
        f.write(datas)
        f.write('\n')
        f.close()
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        np.save('./Wav2vec_LSTM_training_losses.npy', A)
        np.save('./Wav2vec_LSTM_training_scores.npy', B)
        np.save('./Wav2vec_LSTM_test_loss.npy', C)
        np.save('./Wav2vec_LSTM_test_score.npy', D)
