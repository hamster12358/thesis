...
Original file is located at
    https://colab.research.google.com/drive/1M6WtpBUJzo4ovcygoDI9FCwvMT4N2BUn
"""
...
class EncoderCNN(nn.Module):
    def __init__(self, img_x=120, img_y=120, fc_hidden1=512, fc_hidden2=512, drop_p=0.3, CNN_embed_dim=300):
...
save_model_path = "./2dcnnmodel"  # 设置保存模型路径
...
def validation(model, device, optimizer, test_loader):
    ...
    with open('2dcnndatas.txt', 'a', encoding='utf-8') as f:
        f.write(datas)
        f.write('\n')
        f.close()
...
def plot_confusion_matrix(gt_labels, pred_labels, normalize=True, title='Confusion matrix', cmap=plt.cm.Blues):
    ...
    plt.savefig('2dcnn_cm.jpg')


# def plt_cm(gt_labels, pred_labels):
...
if __name__ == '__main__':
    for epoch in range(epochs):
        ...
        np.save('./2DCNN_epoch_training_losses.npy', A)
        np.save('./2DCNN_epoch_training_scores.npy', B)
        np.save('./2DCNN_epoch_test_loss.npy', C)
        np.save('./2DCNN_epoch_test_score.npy', D)
    # plt_cm(all_y.cpu(), all_y_pred.cpu()[:, 0])
